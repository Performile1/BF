...
export const CustomizableBentoDashboard: React.FC<CustomizableBentoDashboardProps> = ({
  ...
  const renderWidgetContent = (id: string) => {
    switch (id) {
      ...
      case 'who_is_at_hub':
        const checkedInMembers = members.filter(m => m.id !== currentUser.id).slice(0, 3);
        return (
          <div className="flex flex-col justify-between h-full space-y-3">
            <div>
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="space-y-2">
                {checkedInMembers.map((m, idx) => (
                  <div
                    key={m.id}
                    className="p-2.5 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between gap-3 hover:bg-gray-100/70 transition"
                  >
